...
from typing import Annotated, Any, TypeVar, Union, get_origin
...
from pydantic import BaseModel as _BaseModel, ConfigDict, Field, GetCoreSchemaHandler, GetJsonSchemaHandler
...
@lru_cache(maxsize=None)
def GetProjection(_cls: Union[Annotated, BaseModel]) -> dict[str, int]: # type: ignore
    if isinstance(_cls, BaseModel):
        return _cls.Projection()
    ret: dict[str, int] = {} 
    if (get_origin(_cls) == Annotated) and (_cls.__getattribute__("__metadata__")):
        annotatedArgs = _cls.__args__
        annotatedArg = annotatedArgs[0]

        unionArgs = annotatedArg.__args__
        for unionArg in unionArgs:
            if issubclass(unionArg, BaseModel):
                p = unionArg.Projection()
                ret.update(p)
    return ret
...
def BsonRegexEncode(r: BsonRegex[Any]) -> dict[str, Any]:
    return {
        "pattern": r.pattern,
        "flags": r.flags
    }
