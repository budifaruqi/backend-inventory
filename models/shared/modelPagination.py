...
class MsPaginationSort(BaseModel):
    sortby: str = Field(
        default=...,
        max_length=50,
        description="Urutkan berdasarkan nama kolom",
...
class MsPagination2(BaseModel):
    ...
    sort: Optional[List[MsPaginationSort]] = Body(
        default=None,
        title="Sort"
    )
...
class MsPaginationResult2(BaseModel, Generic[TGenericPaginationModel]):

    sort: Optional[List[MsPaginationSort]] = Field(
        default=None,
        title="Sort",
        description="Array urutan nama kolom"
    )
